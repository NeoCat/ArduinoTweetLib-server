...
extern "C" {
   ...
   #include "utility/EthernetDhcpUtil.h"
}
...
int EthernetDHCPClass::begin(uint8_t* macAddr, int pollingMode)
{
   ...
   iinchip_init();
 	setSHAR(this->_dhcpData.macAddr);
 	setSIPR(this->_dhcpData.ipAddr);
 	
 	sysinit(RX_TX_BUF_SIZE, RX_TX_BUF_SIZE);
...
int EthernetDHCPClass::_startDHCPSession()
{
   ...
   for (i = NUM_SOCKETS-1; i>=0; i--)
      if (SOCK_CLOSED == IINCHIP_READ(Sn_SR(i))) {
         if (socket(i, Sn_MR_UDP, DHCP_CLIENT_PORT, 0) > 0) {
            this->_socket = i;
            break;
         }
      }
   ...
   if (DhcpStateRenewing == this->_state || DhcpStateRequesting == this->_state) {
      IINCHIP_WRITE(Sn_DIPR0(this->_socket), this->_dhcpData.serverIpAddr[0]);
      IINCHIP_WRITE((Sn_DIPR0(this->_socket) + 1), this->_dhcpData.serverIpAddr[1]);
      IINCHIP_WRITE((Sn_DIPR0(this->_socket) + 2), this->_dhcpData.serverIpAddr[2]);
      IINCHIP_WRITE((Sn_DIPR0(this->_socket) + 3), this->_dhcpData.serverIpAddr[3]);
   } else {
      IINCHIP_WRITE(Sn_DIPR0(this->_socket), 255);
      IINCHIP_WRITE((Sn_DIPR0(this->_socket) + 1), 255);
      IINCHIP_WRITE((Sn_DIPR0(this->_socket) + 2), 255);
      IINCHIP_WRITE((Sn_DIPR0(this->_socket) + 3), 255);
   }
...
int EthernetDHCPClass::_sendDhcpMessage(DhcpMessageType_t type)
{
   ...
   bootpPacket->xid = dhcp_htonl(this->_dhcpData.xid);
   
   bootpPacket->secs = dhcp_htons((now - this->_sessionBeginMillis) / 1000);
   bootpPacket->flags = dhcp_htons(DhcpFlagNone | DhcpFlagBroadcast);
   ...
   write_data(this->_socket, (vuint8*)bootpPacket, (vuint8*)ptr, sizeof(BootpPacket_t));
   ...
   while (cnt > 0) {
      ...
      write_data(this->_socket, (vuint8*)bootpPacket, (vuint8*)ptr, sndBytes);
      ptr += sndBytes;
      cnt -= sndBytes;
   }
   ...
   // OPTION: magic cookie
   *((uint32_t*)buf) = dhcp_htonl(DHCP_OPTS_MAGIC_COOKIE);
   ...
   ptr += 2;
   write_data(this->_socket, (vuint8*)hostName, (vuint8*)ptr, strlen((const char*)hostName));
   ...
   if (this->_useHostNameAsClientIdentifier) {
      ...
      write_data(this->_socket, (vuint8*)buf, (vuint8*)ptr, 3 + sizeof(this->_dhcpData.macAddr));
   ...
   if (DhcpMessageRequest == type && DhcpStateRenewing != this->_state) {
      ...
      write_data(this->_socket, (vuint8*)buf, (vuint8*)ptr, 12);
      ptr += 12;
   }
...
DhcpMessageType_t EthernetDHCPClass::_processDhcpReply()
{
   ...
   uint32_t magicCookie = dhcp_htonl(DHCP_OPTS_MAGIC_COOKIE);
   ...
   *((uint16_t*)&svr_port) = dhcp_ntohs(*((uint32_t*)(buf+4)));
   *((uint16_t*)&udp_len) = dhcp_ntohs(*((uint32_t*)(buf+6)));
   
   read_data(this->_socket, (vuint8*)ptr, (vuint8*)bootpPacket, sizeof(BootpPacket_t));
   ...
   if (BootpReply == bootpPacket->opCode && DHCP_SERVER_PORT == svr_port &&
       ...
       dhcp_ntohl(bootpPacket->xid) == this->_dhcpData.xid) {
         
         // check for magic cookie
         read_data(this->_socket, (vuint8*)ptr + 236, (vuint8*)buf, sizeof(uint32_t));
         if (0 == memcmp(buf, &magicCookie, sizeof(uint32_t))) {
            ...
            read_data(this->_socket, (vuint8*)ptr + 240, (vuint8*)buf, opt_len);
            ...
            while (p < e) {
               ...
               switch (*p++) {
                  ...
                  case DhcpOptionIPAddrLeaseTime:
                     this->_dhcpData.leaseTime = dhcp_ntohl(*((uint32_t*)(p+1)));
                     break;
                  case DhcpOptionT1value:
                     this->_dhcpData.t1 = dhcp_ntohl(*((uint32_t*)(p+1)));
                     break;
                  case DhcpOptionT2value:
                     this->_dhcpData.t2 = dhcp_ntohl(*((uint32_t*)(p+1)));
                     break;
         ...
         switch (this->_state) {
            ...
            case DhcpStateRenewing:
               if (type == DhcpMessageAck) {
                  ...
                  setSIPR(this->_dhcpData.ipAddr);
                 	setGAR(this->_dhcpData.gatewayIpAddr);
                  setSUBR(this->_dhcpData.subnetMask);
